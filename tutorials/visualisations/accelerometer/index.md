...
This is perhaps easier to understand by example.  uibuilder has a concept of **birth** and **death**. Click on the yaxis circle and then click on 'lifetime'.  You are given two options "bind to key" and "bind to function".  Click on "bind to key", now select "TS".  We are telling uibuilder that whenever it sees a new "ts" value (which it will with every new data item), rather than modify the current circle, clone a new one.  Lets try this and see what happens.  Click on OK then click on test and wait for the container to build.  Now when you click on the app icon to see the test data, you should see something like:
...
So a new yaxis circle is created with every new timestamp, whereas the xaxis circle behaves as before.  This looks nice, but creates a load of circles.  We want to clean up old ones.  We can do this by setting the death options.  The death options is a function that is evaluated with every new item of data received.  It passes in the svg object and returns true if the object should be removed, and false otherwise.  Set the death function to:

```javascript
return (Date.now() - node.ts) > 500 
```

i.e. remove the node if it is older than half a second.  This might not look like much against the test data, but will look better when data is arriving much faster.  This is what will create the vapour trail effect.  To create the same for the xaxis circle, repeat the steps above (i.e bind to the ts key for the birth options, and add in the code to the death options.
